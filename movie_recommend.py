...
def getSimModle(): # 默认载入movielens数据集
    ...
    trainset = data.build_full_trainset() #使用pearson_baseline方式计算相似度  False以item为基准计算相似度 本例为电影之间的相似度
...
def read_item_names():
    ...
    """
    file_name = ('D:/PythonProjects/FRSB_on_surprise/ml-1m/movies.dat')
    rid_to_name = {}
    name_to_rid = {}
...
def showSimilarMovies(algo, rid_to_name, name_to_rid):
    ...
    for movie in neighbors_movies:
        print(movie)
if __name__ == '__main__': # 获取id到name的互相映射
